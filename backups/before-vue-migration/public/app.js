...
// ==================== 生成诗歌 ====================
async function generatePoem(song) {
  if (!poemContent) return;

  // 显示加载状态
  poemContent.innerHTML = '<div class="poem-placeholder">正在生成诗歌...</div>';
  ...
  try {
    ...
    if (data.poem) {
      poemContent.innerHTML = `<div class="poem-text">${escapeHtml(data.poem)}</div>`;
    } else {
      // 如果后端不支持，使用本地生成
      const localPoem = generateLocalPoem(song);
      poemContent.innerHTML = `<div class="poem-text">${escapeHtml(localPoem)}</div>`;
    }
  } catch (error) {
...
// ==================== 本地诗歌生成（后备方案）====================
function generateLocalPoem(song) {
  const templates = [
