...
export class Scheduler {
  ...
  async morningRoutine() {
    try {
      ...
      const decision = await this.claude.decide(context);
  ...
  async hourlyCheck() {
    try {
      ...
      const lastPlayTime = recentPlays[0]?.played_at;
      ...
      const timeSinceLastPlay = Date.now() - new Date(lastPlayTime).getTime();
