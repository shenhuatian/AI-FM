...
详见：[推荐系统优化文档](RECOMMENDATION-UPGRADE.md)
...
**可选配置：**
```env
XIAOMI_API_KEY=你的小米 MiMo API Key（语音合成，限时免费）
...
│   │   ├── langchain-adapter.js # LangChain适配器（对话记忆）
│   │   ├── state.js             # 状态管理
│   │   ├── router.js            # 路由分流器
│   │   ├── context.js           # 上下文构建器
│   │   ├── scheduler.js         # 定时调度器
│   │   ├── tts-xiaomi.js        # 小米TTS语音合成
│   │   ├── proactive.js         # 主动对话系统
...
├── test-recommendation.js        # 🆕 推荐系统测试脚本
├── .env.example                  # 环境变量模板
├── package.json
└── README.md
```
